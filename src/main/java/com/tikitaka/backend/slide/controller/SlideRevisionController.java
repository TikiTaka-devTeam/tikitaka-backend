...
import com.tikitaka.backend.slide.dto.InsertSlideRequest;
...
public class SlideRevisionController {
    ...
    @PostMapping("/documents/{document_id}/slides/insert")
    @Operation(
            summary = "수정 강의자료의 슬라이드를 기존 슬라이드 사이에 삽입 연결",
            description = "교수 사용자가 수정 업로드된 슬라이드를 기존 슬라이드 뒤에 삽입하도록 연결합니다. 실제 반영은 revision complete 전까지 보류됩니다."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "슬라이드 삽입 연결 성공",
                    content = @Content(schema = @Schema(implementation = InsertSlideResponse.class))
            ),
    ...
    public ResponseEntity<InsertSlideResponse> insertSlide(
            ...
            @RequestBody @Valid InsertSlideRequest request
    ) {
        ...
        InsertSlideResponse response = slideRevisionService.insertSlide(
                professorId,
                documentId,
                request.revisionSlideId(),
                request.insertAfterSlideId()
        );
